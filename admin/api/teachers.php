...
ini_set('error_log', __DIR__ . '/teachers_errors.log');
...
try {
    ...
    switch ($action) {
        case 'get':
            ...
            sendJsonResponse(true, 'Lista de professores', $stmt->fetchAll());
            break;
        ...
        case 'create':
            ...
            $stmt = $conn->prepare("INSERT INTO teachers 
                (fname, lname, username, gender, dob, bi_number, address, contact, email, password) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            ...
            sendJsonResponse(true, 'Professor criado com sucesso', ['id' => $conn->lastInsertId()], 201);
            break;
        ...
        case 'update':
            ...
            if (empty($input['id'])) {
                throw new Exception("ID do professor não fornecido", 400);
            }

            $requiredFields = ['fname', 'lname', 'username', 'contact', 'dob', 'bi_number'];
            ...
            if ($age < 18) {
                throw new Exception("O professor precisa ter pelo menos 18 anos.", 400);
            }
            ...
            sendJsonResponse(true, 'Professor atualizado com sucesso', ['id' => $input['id']]);
            break;
        ...
        case 'delete':
            ...
            $stmt = $conn->prepare("SELECT id FROM teachers WHERE id = :id");
            $stmt->execute([':id' => $id]);
            ...
            sendJsonResponse(true, 'Professor excluído com sucesso');
            break;
